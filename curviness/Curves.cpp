...
float algorithms::linear_interpolation(float t, float a, float b) {
	return a + t * (b - a);
}

float algorithms::quadratic_bezier(float t, float a, float b, float c) {
	float one_minus_t = 1 - t;
	return a * one_minus_t*one_minus_t + b * 2 * one_minus_t * t + c * t*t;
}

float algorithms::cubic_bezier(float t, float a, float b, float c, float d) {
	float one_minus_t = 1 - t;
